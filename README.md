...
비동기 작업은 escaping closure로 처리되므로, 만약 Bank를 struct로 정의하면 자신의 프로퍼티 변경을 비동기로 수행할 수 없게 됩니다. 이럴 경우, 메인 스레드가 UI 외에 해야할 작업이 늘어나게 되고, 구현 복잡도가 많이 올라가므로 득실을 따져야 했습니다
